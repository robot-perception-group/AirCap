...
class Syncer:
	...
	def __init__(self, topic1, topic2, time_window):
	...
	def _publish(self):
		...
		if abs(self._last_msg1.header.stamp - self._last_msg2.header.stamp) < self._time_window:
			...
			X=self._last_msg2.position.x-self._last_msg1.position.x
			Y=self._last_msg2.position.y-self._last_msg1.position.y
			Z=self._last_msg2.position.z-self._last_msg1.position.z
			XY=math.sqrt(X*X+Y*Y)
			XYZ=math.sqrt(X*X+Y*Y+Z*Z)
			print("%f\t%f\t%f\t%f\t%f\t%f"%(self._last_msg1.header.stamp,X,Y,Z,XY,XYZ))


if __name__ == "__main__":

	if len(sys.argv) < 4:
		print("Too few arguments! Usage: sync_node.py topic1 topic2 time_window")
	else:
		print("#[Timestamp]\tX\tY\tZ\t2D\t3D")
		syncer = Syncer(sys.argv[1], sys.argv[2], sys.argv[3])
