...
class copenet(nn.Module):
    ...
    def forward(self, x0, x1,
        ...
        if init_theta0 is None:
            init_orient0 = self.init_pose[:,:6].expand(batch_size,-1)
            init_art_pose0 = self.init_pose[:,6:22*6].expand(batch_size, -1)
        else:
        ...
        if init_theta1 is None:
            init_orient1 = self.init_pose[:,:6].expand(batch_size,-1)
            init_art_pose1 = self.init_pose[:,6:22*6].expand(batch_size, -1)
        else:
        ...
        xf0 = self.forward_feat_ext(x0)
        xf1 = self.forward_feat_ext(x1)
    ...
    def forward_reg(self, xf0,
        ...
        pred_shape0 = pred_shape0 + self.decshape(xc0)
        pred_pose0 = torch.cat([pred_position0, pred_orient0, pred_art_pose0],1) + self.decpose(xc0)

        return pred_pose0, pred_shape0

def getmodel(smpl_mean_params_path):
    model = copenet(Bottleneck, [3, 4, 6, 3],  smpl_mean_params_path)
    return model
