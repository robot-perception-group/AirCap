...
class RunModel(object):
    def __init__(self, config, num_views, num_true_views, sess=None):
        ...
        load_path = aircaprl_path+'/git/HumanMultiView/models/model.ckpt-55124'                
        ...
        if not exists(load_path + '.index'):
            print('%s doesnt exist..' % load_path)
            import ipdb
            ipdb.set_trace()
        ...
        self.global_orient = tf.placeholder(tf.float32, shape=[1, 3])
    ...
    def build_test_model_ief(self):
        ...
        for i in range(0, self.num_views):
            ...
            poses = tf.concat([self.global_orient,theta_here[:, self.num_cam+3:(self.num_cam+ self.num_theta)]],1)
    ...
    def predict(self, images, rot_w,get_theta=False):
        """
        images: num_batch, img_size, img_size, 3
        Preprocessed to range [-1, 1]
        """
        results = self.predict_dict(images,rot_w)
    ...
    def predict_dict(self, images,rot_w):
        """
        images: num_batch, img_size, img_size, 3
        Preprocessed to range [-1, 1]
