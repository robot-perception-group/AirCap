...
for i in $(seq 0 $(($BLIMPS-1))); do
    ...
    screen -d -m -S AIRCAP$id bash -i -c "roslaunch aircap simulation_blimp.launch librepilotUseUDP:=False robotID:=$id numRobots:=$ROBOS comSuccessRate:=$COMSUCCESSRATE Z:=8 X:=${Xs[$id]}  Y:=${Ys[$id]} --screen"
    sleep 5
done
